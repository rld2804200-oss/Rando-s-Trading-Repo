...
DEFAULTS = {
    ...
    "BYPASS_ORDER_SIZE_CHECKS": False,
...
BUY_ONLY_MODE = config["BUY_ONLY_MODE"]
SELL_ONLY_MODE = config["SELL_ONLY_MODE"]
ONLY_BUY_HIGHER = config["ONLY_BUY_HIGHER"]
ONLY_BUY_LOWER = config["ONLY_BUY_LOWER"]
USE_SL = config["USE_SL"]
USE_TP = config["USE_TP"]
USE_PERCENT_TP_SL = config["USE_PERCENT_TP_SL"]
...
log(Fore.CYAN + "TVWebHook_2.0.60")
...
def fetch_ticker_with_retry(ex, symbol, max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                log(Fore.YELLOW + f"Phemex API unavailable, retrying... (attempt {attempt + 1}/{max_retries})")
                time.sleep(2)
            else:
                raise
        except Exception:
...
def process_single_symbol(symbol, side, lotsize):
    ...
    # --- LONG block: ONLY_BUY_LOWER ---
    if ONLY_BUY_LOWER and side == 'buy':
        positions = exchange.fetch_positions([symbol])
        entry_price = None
        for position in positions:
        ...
        if entry_price:
            trade_price = limit_price if (
                        USE_LIMIT_ORDERS and 'limit_price' in locals() and limit_price) else get_current_price(
                symbol)
            if trade_price is not None and trade_price >= entry_price:
                ...
                return None, "Trade price not lower than open long — skipped per ONLY_BUY_LOWER"

                # --- SHORT block: ONLY_BUY_HIGHER ---
    if ONLY_BUY_HIGHER and side == 'sell':
        positions = exchange.fetch_positions([symbol])
        entry_price = None
        for position in positions:
            if position['symbol'] == symbol and float(position.get('contracts', 0) or 0) != 0:
        ...
        if entry_price:
            ...
            if trade_price is not None and trade_price <= entry_price:
                ...
                return None, "Trade price not higher than open short — skipped per ONLY_BUY_HIGHER"
...
def browser_visit():
    return "RanBot TVWebHook_2.0.60 is online", 200
