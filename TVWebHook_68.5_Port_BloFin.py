...
CONFIG_FILE = os.path.join(base_path, "configFile_BloFin.json")
...
DEFAULTS = {
    "PC_NAME": "MSI_1",                              # Name of this computer (shows in every log)
    "SUBACCOUNT_NAME": "BloFin Acc.",                   # Name of the sub-account (shows in every log)
    "EXCHANGE": "blofin",                          # Exchange name for CCXT
    "REFUSE_SYMBOLS": ["XRP/USD:USD"],
    "FORWARD_REFUSED_TO": "https://ranbot.site/",
    ...
    "WATCH_SYMBOLS": [ ],                          # Symbols checked at startup + auto-learned
    ...
    "FIXED_TP_PRICE_SELL": 55555,                  # Fixed TP price for sells
    ...
    "SL_TRIGGER_POINTS": 500.0,                    # Stop Loss in points (when SL_MODE = "points")
    "FIXED_SL_PRICE_BUY": 55555,                   # Fixed SL price for buys
    ...
    "ENV_FILE": "BloFin_Coin_M.env",                   # File that holds the API keys
...
PC_NAME = config.get("PC_NAME", "MSI_1")
SUBACCOUNT_NAME = config.get("SUBACCOUNT_NAME", "BloFin Main Acc:")
EXCHANGE = config["EXCHANGE"]
REFUSE_SYMBOLS = config.get("REFUSE_SYMBOLS", [])
FORWARD_REFUSED_TO = config.get("FORWARD_REFUSED_TO", "https://ranbot.site/")
ALWAYS_FORWARD = config.get("ALWAYS_FORWARD", False)
ENABLE_FORWARDING = config.get("ENABLE_FORWARDING", True)
SYMBOL = config["SYMBOL"]
WATCH_SYMBOLS = config.get("WATCH_SYMBOLS", [])
...
log(Fore.CYAN + "TVWebHook_1.0.68")
...
try:
    temp_exchange = getattr(ccxt, EXCHANGE)({
        'apiKey': api_key,
        'secret': api_secret,
        'password': api_password,  # required for BloFin
...
def process_single_symbol(symbol, side, lotsize):
    ...
    # Inverse / coin-m: settle currency = base (BTC/USD:BTC, ETH/USD:ETH, ...)
    try:
        market = exchange.market(symbol)
        ...
        if len(parts) >= 3 and parts[0].upper() == parts[-1].upper():
            min_size = max(min_size, 1.0)

    log(Fore.CYAN + f"Exchange min size for {symbol}: {min_size}")
    if lotsize < min_size:
    ...
    try:
        ...
        if '11058' in err or 'TE_QTY_TOO_SMALL' in err or 'too small' in err.lower():
            log(Fore.YELLOW + "⚠️ Order size still too small after applying exchange minimum.")
            log(Fore.YELLOW + "   Symbol may need a special rule for this exchange.")
            log(Fore.YELLOW + "   Contact the publisher for an update if this keeps happening.")
            return None, "TE_QTY_TOO_SMALL after min-size adjust — contact publisher if persistent"

        if 'BadSymbol' in err or 'does not have market symbol' in err.lower() or 'invalid symbol' in err.lower():
            log(Fore.YELLOW + f"⚠️ Symbol '{symbol}' not recognized by the exchange.")
            log(Fore.YELLOW + "   Check the symbol format or contact the publisher for an update.")
            return None, f"Bad symbol: {symbol}"

        if '11082' in err or 'TE_CANNOT_COVER_ESTIMATE_ORDER_LOSS' in err or 'insufficient' in err.lower():
            # Guess margin currency from symbol
            if ':USDT' in symbol or symbol.endswith('/USDT'):
                margin_coin = 'USDT'
            elif ':USD' in symbol and ':BTC' in symbol:
                margin_coin = 'BTC'
            elif ':ETH' in symbol:
                margin_coin = 'ETH'
            else:
                parts = symbol.replace(':', '/').split('/')
                margin_coin = parts[-1] if parts else 'margin currency'

            log(Fore.YELLOW + "⚠️ Not enough margin (or wrong currency) for this order.")
            log(Fore.YELLOW + f"   This symbol needs free {margin_coin} for margin, not only USDT.")
            log(Fore.YELLOW + f"   Check your {margin_coin} balance on the exchange (testnet/live).")
            return None, f"Insufficient margin — need free {margin_coin}"
...
def browser_visit():
    return "RanBot TVWebHook_1.0.68 is online", 200
...
if __name__ == '__main__':
    ...
    print(Fore.CYAN + f"Version: 1.0.68")
